...
function updateGameStep(
    ...
    const updatedTowers = prev.towers.map(tower => {
        ...
        if (newTower.cooldown <= 0) {
            ...
            if (mainTargets.length > 0) {
                ...
                if (tId.includes('missile') || tId.includes('rocket') || tId.includes('javelin') || tId.includes('patriot') || tId.includes('s400')) {
                ...
                } else if (tId.includes('b21') || newTower.type === 'b21_bomber') {
                    // B-21 Raider Bomber - Spawn as plane instead of projectile
                    newPlanes.push({
                        id: crypto.randomUUID(),
                        x: newTower.x,
                        y: newTower.y,
                        ...
                        speed: 4, // Slower, heavier bomber
                        status: 'outbound',
                        type: 'b21_bomber',
                        payloadTimer: 0,
                        config: newTower
                    });
                    newTower.cooldown = newTower.rate * 2; // Longer cooldown for sortie
                    projectileType = 'none'; // Skip default projectile
                } else if (tId.includes('b2') || newTower.type === 'b2_bomber' || newTower.type === 'b2_spirit') {
                    ...
                    newTower.cooldown = newTower.rate * 2;
                    projectileType = 'none';
                } else if (newTower.type?.includes('gunship') || newTower.type?.includes('c130')) {
                    // C-130 Gunship
                    newPlanes.push({
                        id: crypto.randomUUID(),
                        x: newTower.x,
                    ...
                    newTower.cooldown = newTower.rate * 2;
                    projectileType = 'none';
                } else if (newTower.type?.includes('helicopter') || newTower.type?.includes('ah64')) {
                    // Attack Helicopter
                    newPlanes.push({
                        id: crypto.randomUUID(),
                        x: newTower.x,
                    ...
                    newTower.cooldown = newTower.rate * 2;
                    projectileType = 'none';
                } else if (newTower.type?.includes('tiltrotor') || tId.includes('osprey')) {
                    // V-22 Osprey
                    newPlanes.push({
                        id: crypto.randomUUID(),
                        x: newTower.x,
                    ...
                    projectileType = 'none'; // Skip default projectile
                } else if (tId.includes('tank') || tId.includes('abrams') || tId.includes('leopard') || tId.includes('challenger') || tId.includes('type99') || tId.includes('type10') || newTower.type?.includes('tank') || newTower.type?.includes('vehicle')) {
                    // Spawn Tank Vehicle instead of Projectile (including new tank types)
                    newVehicles.push({
                        idInGame: crypto.randomUUID(),
                        x: newTower.x,
                        y: newTower.y,
                        currentHp: newTower.damage * 10 || 1000, // Tank HP based on damage
                        totalHp: newTower.damage * 10 || 1000,
                        pathIndex: 0,
                        active: true,
                        angle: Math.atan2(mainTarget.y - newTower.y, mainTarget.x - newTower.x),
                        targetEnemyId: mainTarget.idInGame,
                        cooldown: 0,
                        // DeployableUnitData fields
                        id: tId,
                        name: newTower.name || 'Tank',
                        cost: newTower.cost || 600,
                        speed: 2, // Tank speed
                        hp: newTower.damage * 10 || 1000,
                        damage: newTower.damage || 80,
                        range: newTower.range || 150,
                        iconUrl: newTower.iconUrl || '',
                        iconHint: newTower.iconHint || 'Tank',
                        // Vehicle config
                        config: newTower
                    });
                    newTower.cooldown = newTower.rate * 3; // Longer cooldown for tank deployment
                    projectileType = 'none'; // Skip default projectile
                }
    ...
    newPlanes = newPlanes.map(plane => {
        ...
        if (dist < p.speed) {
            ...
            if (p.status === 'outbound') {
                ...
                if (p.type === 'a10_warthog') {
                ...
                } else if (p.type === 'b21_bomber') {
                    // B-21 Raider drops massive bomb and returns to base
                    p.status = 'inbound';
                    ...
                    newProjectiles.push({
                        ...
                        damage: p.config.damage || 2000,
                        splash: p.config.splash || 400,
                        active: true,
                        type: 'bomb_b21', // Special type for B-21 mega bombs
    ...
    // 3.5. Update Vehicles (Tanks driving across map)
    let newVehicles = [...(prev.deployedTanks || [])];
    newVehicles = newVehicles.map(vehicle => {
        ...
        const enemiesInRange = updatedEnemies.filter(e =>
            Math.hypot(e.x - v.x, e.y - v.y) <= (v.config.range || 150)
        );

        if (enemiesInRange.length > 0) {
            // Attack nearest enemy
            const nearestEnemy = enemiesInRange.sort((a, b) =>
                Math.hypot(a.x - v.x, a.y - v.y) - Math.hypot(b.x - v.x, b.y - v.y)
            )[0];

            v.angle = Math.atan2(nearestEnemy.y - v.y, nearestEnemy.x - v.x);
            ...
            if (v.cooldown <= 0) {
                newProjectiles.push({
                    ...
                    target: nearestEnemy,
                    speed: 8,
                    damage: v.config.damage || 80,
                    splash: v.config.splash || 0,
                    active: true,
                    type: 'shell',
                    config: v.config as any
                });
                v.cooldown = v.config.rate || 90;
            }

            // Stay in patrolling mode when enemy is in range
            v.status = 'patrolling';
        } else {
            // No enemies in range, move towards a target point or patrol
            if (v.status === 'patrolling') {
                // Set a patrol target (move towards center of map or random direction)
                const centerX = GAME_CONFIG.GRID_WIDTH / 2;
                const centerY = GAME_CONFIG.GRID_HEIGHT / 2;
                v.target = { x: centerX, y: centerY };
                v.status = 'outbound';
            }
            ...
            if (dist < v.speed) {
                ...
                v.target = {
                    x: v.start.x + (Math.random() * 400 - 200),
                    y: v.start.y + (Math.random() * 400 - 200)
                };
                v.status = 'patrolling';
            } else {
                // Move towards target
        ...
        // Keep vehicle on map
        v.x = Math.max(0, Math.min(GAME_CONFIG.GRID_WIDTH, v.x));
        v.y = Math.max(0, Math.min(GAME_CONFIG.GRID_HEIGHT, v.y));
    ...
    remainingProjectiles.forEach(p => {
        ...
        if (hasDetonated) {
            ...
            if (p.splash > 0 || p.type === 'missile' || p.type === 'shell' || p.type === 'sortie' || p.type === 'a10_strafe' || p.type === 'bomb_heavy' || p.type === 'bomb_b21') {
                const isB21Bomb = p.type === 'bomb_b21';
                newExplosions.push({
                    id: crypto.randomUUID(),
                    x: detonationX,
                    y: detonationY,
                    radius: isB21Bomb ? (p.splash * 0.15) : (p.splash > 0 ? p.splash * 0.1 : 10),
                    maxRadius: isB21Bomb ? (p.splash * 1.2) : (p.splash > 0 ? p.splash : 40),
                    life: 0.8,
                    color: isB21Bomb ? '#FF4500' : (p.type === 'energy' ? '#00FFFF' : '#FFA500')
                });
            }
